...
def validate_catalog_envelope(
    ...
    *,
    ...
    if not isinstance(envelope, dict):
        raise CatalogError(f"Catalog envelope must be an object{location}")
    missing = ENVELOPE_FIELDS - set(envelope)
    if missing:
        raise CatalogError(f"Catalog envelope is missing {sorted(missing)}{location}")
    if not isinstance(envelope["schemaVersion"], int) or envelope["schemaVersion"] < 1:
        raise CatalogError(f"Invalid schemaVersion{location}")
    ...
    if (
        ...
        raise CatalogError(f"Invalid generator metadata{location}")
    source_files = envelope["sourceFiles"]
    if not isinstance(source_files, list):
        raise CatalogError(f"sourceFiles must be an array{location}")
    source_names: set[str] = set()
    for source in source_files:
        if not isinstance(source, dict) or set(source) != {"name", "sha256"}:
            raise CatalogError(f"Invalid sourceFiles entry{location}")
        name = source.get("name")
        sha256 = source.get("sha256")
        if not isinstance(name, str) or not name or name in source_names:
            raise CatalogError(f"Invalid or duplicate source file name {name!r}{location}")
        if not _is_sha256(sha256):
            raise CatalogError(f"Invalid source file sha256 for {name!r}{location}")
    ...
    if (
        ...
        raise CatalogError(f"supportedScenarios must contain unique scenario names{location}")
    if not isinstance(envelope["base"], dict):
        raise CatalogError(f"base must be an object{location}")
    overrides = envelope["scenarioOverrides"]
    if not isinstance(overrides, dict):
        raise CatalogError(f"scenarioOverrides must be an object{location}")
    unsupported_overrides = sorted(set(overrides) - set(scenarios))
    if unsupported_overrides:
        raise CatalogError(f"Scenario overrides are not declared as supported: {unsupported_overrides}{location}")
    if any(not isinstance(value, dict) for value in overrides.values()):
        raise CatalogError(f"Every scenario override must be an object{location}")
    expected = value_fingerprint(envelope_payload(envelope))
    if envelope["payloadFingerprint"] != expected:
        raise CatalogError(f"Catalog payload fingerprint mismatch{location}")
    return envelope
...
def select_catalog_scenario(envelope: Mapping[str, Any], scenario: str) -> dict[str, Any]:
    ...
    if not isinstance(scenario, str) or not scenario:
        raise CatalogError("A canonical scenario name is required")
    if not isinstance(scenarios, list) or scenario not in scenarios:
        raise CatalogError(f"Unsupported scenario {scenario!r}; supported scenarios: {scenarios or []}")
    base = envelope.get("base")
...
class RuntimeCatalogs:
    ...
    def load(
        ...
        *,
        ...
        if missing_entries:
            raise CatalogError(f"Catalog manifest is missing required entries {missing_entries}: {manifest_path}")
        ...
        for filename in requested:
            path = root / filename
            if not path.is_file():
                raise CatalogError(f"Packaged catalog is missing: {path}")
            entry = entries[filename]
            actual_sha = file_sha256(path)
            if actual_sha != entry["sha256"]:
                raise CatalogError(f"Catalog file sha256 mismatch: {path}")
            envelope = validate_catalog_envelope(_load_json(path, "catalog"), path=path)
            if envelope["schemaVersion"] != entry["schemaVersion"]:
                raise CatalogError(f"Catalog schemaVersion disagrees with manifest: {path}")
            if envelope["payloadFingerprint"] != entry["payloadFingerprint"]:
                raise CatalogError(f"Catalog payload fingerprint disagrees with manifest: {path}")
    ...
    def ships(self) -> dict[str, dict[str, Any]]:
        ...
        if not isinstance(value, dict):
            raise CatalogError("Loaded catalog bundle has no ship payload")
        return value
    ...
    def nation_claims(self) -> dict[str, Any]:
        ...
        if not isinstance(value, dict):
            raise CatalogError("Loaded catalog bundle has no nation-claim payload")
        return value
    ...
    def _ship_rows(self, collection: str) -> dict[str, dict[str, Any]]:
        ...
        if not isinstance(rows, dict):
            raise CatalogError(f"Loaded ship catalog has no {collection} collection")
        return rows
    ...
    def _rows(self, catalog: str, collection: str) -> dict[str, dict[str, Any]]:
        ...
        if not isinstance(rows, dict):
            raise CatalogError(f"Loaded {catalog} catalog has no {collection} collection")
        return rows
...
def _load_json(path: Path, label: str) -> Any:
    if not path.is_file():
        raise CatalogError(f"Packaged {label} is missing: {path}")
    try:
        return json.loads(path.read_text(encoding="utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise CatalogError(f"Unable to read {label}: {path}: {exc}") from exc


def _validate_manifest(manifest: Any, path: Path) -> dict[str, dict[str, Any]]:
    if not isinstance(manifest, dict) or manifest.get("schemaVersion") != 1:
        raise CatalogError(f"Unsupported catalog manifest schema: {path}")
    generator = manifest.get("generator")
    if not isinstance(generator, dict) or not generator.get("name") or not generator.get("version"):
        raise CatalogError(f"Invalid catalog manifest generator metadata: {path}")
    entries = manifest.get("catalogs")
    if not isinstance(entries, dict) or not entries:
        raise CatalogError(f"Catalog manifest has no catalog entries: {path}")
    for filename, entry in entries.items():
        if (
            ...
            raise CatalogError(f"Invalid catalog manifest entry {filename!r}: {path}")
    expected_bundle = value_fingerprint(entries)
    if manifest.get("bundleFingerprint") != expected_bundle:
        raise CatalogError(f"Catalog manifest bundle fingerprint mismatch: {path}")
    return entries
