# Graph Report - TI_Parser  (2026-08-27)

## Corpus Check
- 124 files · ~670,972 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2286 nodes · 6466 edges · 125 communities (114 shown, 11 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 397 edges (avg confidence: 0.61)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `2bc2bf05`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- [[_COMMUNITY_Save Indexing Core|Save Indexing Core]]
- [[_COMMUNITY_Runtime Catalog System|Runtime Catalog System]]
- [[_COMMUNITY_Nation State Calculations|Nation State Calculations]]
- [[_COMMUNITY_Research Planning Pipeline|Research Planning Pipeline]]
- [[_COMMUNITY_Hab Catalog Builders|Hab Catalog Builders]]
- [[_COMMUNITY_Research Catalog Builder|Research Catalog Builder]]
- [[_COMMUNITY_Nation Projection Engine|Nation Projection Engine]]
- [[_COMMUNITY_Income Dependency Model|Income Dependency Model]]
- [[_COMMUNITY_Projection and Simulation Helpers|Projection and Simulation Helpers]]
- [[_COMMUNITY_Core Projection Tests|Core Projection Tests]]
- [[_COMMUNITY_Organization Planner|Organization Planner]]
- [[_COMMUNITY_Habitat Mechanics|Habitat Mechanics]]
- [[_COMMUNITY_Hab Reference Resolution|Hab Reference Resolution]]
- [[_COMMUNITY_Hab UI and Resources|Hab UI and Resources]]
- [[_COMMUNITY_CLI Save Loading|CLI Save Loading]]
- [[_COMMUNITY_AI Fleet Diagnostics|AI Fleet Diagnostics]]
- [[_COMMUNITY_Topbar and Research UI|Topbar and Research UI]]
- [[_COMMUNITY_Org Planner Tests|Org Planner Tests]]
- [[_COMMUNITY_Hab Module Planning|Hab Module Planning]]
- [[_COMMUNITY_Research UI Tests|Research UI Tests]]
- [[_COMMUNITY_Module Construction Analysis|Module Construction Analysis]]
- [[_COMMUNITY_Hab Plan Tests|Hab Plan Tests]]
- [[_COMMUNITY_Parser Reliability Design|Parser Reliability Design]]
- [[_COMMUNITY_Package Runtime Migration|Package Runtime Migration]]
- [[_COMMUNITY_Hab Power Tests|Hab Power Tests]]
- [[_COMMUNITY_Org Eligibility Design|Org Eligibility Design]]
- [[_COMMUNITY_Nation Projection Audit|Nation Projection Audit]]
- [[_COMMUNITY_Catalog Verification Tests|Catalog Verification Tests]]
- [[_COMMUNITY_Nation Claims Tests|Nation Claims Tests]]
- [[_COMMUNITY_Org Parity Tests|Org Parity Tests]]
- [[_COMMUNITY_CLI Command Dispatch|CLI Command Dispatch]]
- [[_COMMUNITY_Hab Module Documentation|Hab Module Documentation]]
- [[_COMMUNITY_Scenario Rules Tests|Scenario Rules Tests]]
- [[_COMMUNITY_Ship Planner Tests|Ship Planner Tests]]
- [[_COMMUNITY_Project README Concepts|Project README Concepts]]
- [[_COMMUNITY_Mission Control Projection|Mission Control Projection]]
- [[_COMMUNITY_Package Runtime Audit|Package Runtime Audit]]
- [[_COMMUNITY_Scenario Rule Design|Scenario Rule Design]]
- [[_COMMUNITY_Graphify Memory Model|Graphify Memory Model]]
- [[_COMMUNITY_Parser Core Tests|Parser Core Tests]]
- [[_COMMUNITY_Strict Effect Tests|Strict Effect Tests]]
- [[_COMMUNITY_Hab Slot Tests|Hab Slot Tests]]
- [[_COMMUNITY_Package Runtime Tests|Package Runtime Tests]]
- [[_COMMUNITY_Project Analysis Tests|Project Analysis Tests]]
- [[_COMMUNITY_Research Plan Tests|Research Plan Tests]]
- [[_COMMUNITY_Recommendation Evidence Design|Recommendation Evidence Design]]
- [[_COMMUNITY_Project Tooling Configuration|Project Tooling Configuration]]
- [[_COMMUNITY_Hab Parser Parity Tests|Hab Parser Parity Tests]]
- [[_COMMUNITY_Raw Loader Guard Tests|Raw Loader Guard Tests]]
- [[_COMMUNITY_Broken Earth Greece Query|Broken Earth Greece Query]]
- [[_COMMUNITY_Income Wrapper Tests|Income Wrapper Tests]]
- [[_COMMUNITY_CLI Namespace|CLI Namespace]]
- [[_COMMUNITY_Community 52|Community 52]]
- [[_COMMUNITY_Community 53|Community 53]]
- [[_COMMUNITY_Community 54|Community 54]]
- [[_COMMUNITY_Community 55|Community 55]]
- [[_COMMUNITY_Community 56|Community 56]]
- [[_COMMUNITY_Community 57|Community 57]]
- [[_COMMUNITY_Community 58|Community 58]]
- [[_COMMUNITY_Community 59|Community 59]]
- [[_COMMUNITY_Community 60|Community 60]]
- [[_COMMUNITY_Community 61|Community 61]]
- [[_COMMUNITY_Community 62|Community 62]]
- [[_COMMUNITY_Community 63|Community 63]]
- [[_COMMUNITY_Community 64|Community 64]]
- [[_COMMUNITY_Community 65|Community 65]]
- [[_COMMUNITY_Community 66|Community 66]]
- [[_COMMUNITY_Community 67|Community 67]]
- [[_COMMUNITY_Community 68|Community 68]]
- [[_COMMUNITY_Community 69|Community 69]]
- [[_COMMUNITY_Community 70|Community 70]]
- [[_COMMUNITY_Community 71|Community 71]]
- [[_COMMUNITY_Community 72|Community 72]]
- [[_COMMUNITY_Community 73|Community 73]]
- [[_COMMUNITY_Community 74|Community 74]]
- [[_COMMUNITY_Community 75|Community 75]]
- [[_COMMUNITY_Community 76|Community 76]]
- [[_COMMUNITY_Community 77|Community 77]]
- [[_COMMUNITY_Community 78|Community 78]]
- [[_COMMUNITY_Community 79|Community 79]]
- [[_COMMUNITY_Community 80|Community 80]]
- [[_COMMUNITY_Community 81|Community 81]]
- [[_COMMUNITY_Community 82|Community 82]]
- [[_COMMUNITY_Community 83|Community 83]]
- [[_COMMUNITY_Community 84|Community 84]]
- [[_COMMUNITY_Community 85|Community 85]]
- [[_COMMUNITY_Community 86|Community 86]]
- [[_COMMUNITY_Community 87|Community 87]]
- [[_COMMUNITY_Community 88|Community 88]]
- [[_COMMUNITY_Community 89|Community 89]]
- [[_COMMUNITY_Community 90|Community 90]]
- [[_COMMUNITY_Community 91|Community 91]]
- [[_COMMUNITY_Community 92|Community 92]]
- [[_COMMUNITY_Community 93|Community 93]]
- [[_COMMUNITY_Community 94|Community 94]]
- [[_COMMUNITY_Community 95|Community 95]]
- [[_COMMUNITY_Community 96|Community 96]]
- [[_COMMUNITY_Community 97|Community 97]]
- [[_COMMUNITY_Community 98|Community 98]]
- [[_COMMUNITY_Community 99|Community 99]]
- [[_COMMUNITY_Community 100|Community 100]]
- [[_COMMUNITY_Community 101|Community 101]]
- [[_COMMUNITY_Community 102|Community 102]]
- [[_COMMUNITY_Community 103|Community 103]]
- [[_COMMUNITY_Community 104|Community 104]]
- [[_COMMUNITY_Community 105|Community 105]]
- [[_COMMUNITY_Community 106|Community 106]]
- [[_COMMUNITY_Community 107|Community 107]]
- [[_COMMUNITY_Community 108|Community 108]]
- [[_COMMUNITY_Community 109|Community 109]]
- [[_COMMUNITY_Community 110|Community 110]]
- [[_COMMUNITY_Community 111|Community 111]]
- [[_COMMUNITY_Community 112|Community 112]]
- [[_COMMUNITY_Community 113|Community 113]]
- [[_COMMUNITY_Community 114|Community 114]]
- [[_COMMUNITY_Community 115|Community 115]]
- [[_COMMUNITY_Community 116|Community 116]]
- [[_COMMUNITY_Community 117|Community 117]]
- [[_COMMUNITY_Community 118|Community 118]]
- [[_COMMUNITY_Community 119|Community 119]]
- [[_COMMUNITY_Community 120|Community 120]]
- [[_COMMUNITY_Community 121|Community 121]]
- [[_COMMUNITY_Community 122|Community 122]]
- [[_COMMUNITY_Community 123|Community 123]]
- [[_COMMUNITY_Community 124|Community 124]]

## God Nodes (most connected - your core abstractions)
1. `IndexedState` - 291 edges
2. `IndexedState` - 188 edges
3. `as_float()` - 187 edges
4. `ref_id()` - 88 edges
5. `NationProjectionState` - 66 edges
6. `clean_numbers()` - 63 edges
7. `CatalogError` - 57 edges
8. `ProjectionContext` - 51 edges
9. `CalculationDependencyError` - 50 edges
10. `CalculationDependency` - 49 edges

## Surprising Connections (you probably didn't know these)
- `Fail-Closed Calculation Dependencies` --semantically_similar_to--> `Unsupported Priority Incomplete Result`  [INFERRED] [semantically similar]
  dev-docs/plan/parser-portability-reliability-2/runtime-dependency-audit.md → docs/plan/nation_projection/02-projection-core.md
- `Data-Only Nation Development Catalog` --semantically_similar_to--> `Packaged Catalog Strategy`  [INFERRED] [semantically similar]
  docs/plan/nation_projection/01-audit-registry.md → dev-docs/plan/ti-parser-reliability/00-master-plan.md
- `Missing org templates fail closed` --conceptually_related_to--> `Fail-closed calculations`  [INFERRED]
  dev-docs/plan/org-eligibility-output/06-recommendation-filtering.md → README.md
- `Sparse Exact Scenario Overrides` --semantically_similar_to--> `Base-Plus-Overlay Named Template Merge`  [INFERRED] [semantically similar]
  docs/research_catalog.md → dev-docs/plan/scenario-specific-rules/02-implementation.md
- `Package-Only Runtime` --semantically_similar_to--> `Audited Package-Only Projection`  [INFERRED] [semantically similar]
  dev-docs/plan/parser-portability-reliability-2/runtime-dependency-audit.md → docs/plan/nation_projection/00-master-plan.md

## Import Cycles
- 1-file cycle: `tools/ti_parser_ai.py -> tools/ti_parser_ai.py`
- 1-file cycle: `tools/ti_save_parser.py -> tools/ti_save_parser.py`
- 1-file cycle: `tools/ti_parser_hab.py -> tools/ti_parser_hab.py`
- 1-file cycle: `tools/ti_parser_nation_projection.py -> tools/ti_parser_nation_projection.py`

## Hyperedges (group relationships)
- **Module-to-Project Unlock Chain** — docs_module_catalog_solar_collector [EXTRACTED 1.00]
- **Auditable Nation Projection Pipeline** — docs_nation_projection_mechanics_audit_shared_mechanics_rule_registry, docs_plan_nation_projection_01_audit_registry_data_only_nation_development_catalog, docs_plan_nation_projection_02_projection_core_projection_model_and_transactional_engine, docs_plan_nation_projection_03_cli_integration_cli_faction_contribution_and_diagnostics, docs_plan_nation_projection_04_verification_rule_linked_one_tick_fixtures [EXTRACTED 1.00]
- **Exact Scenario Data Selection** — dev_docs_plan_scenario_specific_rules_00_master_plan_canonical_scenario_identity, dev_docs_plan_scenario_specific_rules_02_implementation_base_plus_overlay_merge, docs_research_catalog_sparse_exact_scenario_overrides, memory_query_20260814_010906_be_visions_of_greatness_greece_broken_earth_greece_naming_query [INFERRED 0.85]
- **Package-Only Fail-Closed Calculation Data** — dev_docs_plan_parser_portability_reliability_2_runtime_dependency_audit_package_only_runtime, dev_docs_plan_parser_portability_reliability_2_runtime_dependency_audit_fail_closed_dependency_errors, dev_docs_plan_ti_parser_reliability_00_master_plan_packaged_catalog_strategy, dev_docs_plan_ti_parser_reliability_00_master_plan_location_data_fail_closed_contract, docs_plan_nation_projection_01_audit_registry_data_only_nation_development_catalog, docs_plan_nation_projection_02_projection_core_unsupported_priority_incomplete_result [INFERRED 0.95]

## Communities (125 total, 11 thin omitted)

### Community 0 - "Save Indexing Core"
Cohesion: 0.12
Nodes (36): state_value_by_id(), faction_is_active_human(), hab_barycenter_state(), hab_body_is_colonized(), hab_body_is_inhabited(), hab_body_is_irradiated(), hab_body_site_states(), hab_construction_surface_body() (+28 more)

### Community 1 - "Runtime Catalog System"
Cohesion: 0.14
Nodes (32): CatalogVerifyTests, Path, write_json(), build_all(), build_nation_claim_catalog(), build_nation_development_catalog(), build_row_catalog(), build_ship_catalog() (+24 more)

### Community 2 - "Nation State Calculations"
Cohesion: 0.09
Nodes (50): scenario_template_name(), active_owned_control_points(), active_scenario_rules(), average(), calculate_global_public_opinion(), calculate_nation_ui(), calculate_world_atrocities(), calculate_world_ui() (+42 more)

### Community 3 - "Research Planning Pipeline"
Cohesion: 0.10
Nodes (46): first_value(), active_global_research_names(), active_project_names(), active_project_research_names(), active_slots_with_category(), active_slots_with_hypothetical_project_category(), available_global_research_templates(), available_project_research_templates() (+38 more)

### Community 4 - "Hab Catalog Builders"
Cohesion: 0.26
Nodes (19): build_catalog(), build_cost(), build_markdown(), crew_support(), derive_tags(), format_resource_map(), load_module_localizations(), main() (+11 more)

### Community 5 - "Research Catalog Builder"
Cohesion: 0.13
Nodes (42): build_catalog(), build_graph_links(), build_markdown(), canonical_json_bytes(), clean_value(), context_values(), discover_scenario_template_dirs(), discover_supported_scenarios() (+34 more)

### Community 6 - "Nation Projection Engine"
Cohesion: 0.15
Nodes (39): _add_cohesion(), _allowed_armies(), _annual_population_growth(), _apply_completion(), _apply_economy_region_branch(), _apply_effect_context(), _apply_market_mean_path(), _apply_unity_public_opinion() (+31 more)

### Community 7 - "Income Dependency Model"
Cohesion: 0.23
Nodes (32): active_owned_control_points(), councilor_is_income_active(), councilor_monthly_income(), councilor_research_and_mc(), councilor_resource_income(), councilor_yearly_income(), faction_ideology_key(), faction_public_opinion() (+24 more)

### Community 8 - "Projection and Simulation Helpers"
Cohesion: 0.06
Nodes (67): as_float(), clean_numbers(), add_monthly_delta(), bottleneck_penalty_from_delta(), calculate_ship_plan(), faction_excess_mission_control_yearly_income(), faction_stockpile(), hab_module_candidate_rows() (+59 more)

### Community 9 - "Core Projection Tests"
Cohesion: 0.16
Nodes (3): add_state(), ParserReliabilityTests, ref()

### Community 10 - "Organization Planner"
Cohesion: 0.17
Nodes (41): apply_conditional_attribute_mods(), calculate_org_plan(), clamp_attribute(), compare_condition(), condition_eval_unknown(), condition_nation_summary(), councilor_attribute_breakdown(), councilor_org_plan_profile() (+33 more)

### Community 11 - "Habitat Mechanics"
Cohesion: 0.16
Nodes (41): ModuleCatalogError, Raised when authoritative hab-module data cannot be loaded safely., active_modules_in_sectors(), faction_active_org_mining_bonus(), faction_mining_multiplier(), faction_sector_states(), get_effective_module_state(), hab_administration_modifier() (+33 more)

### Community 12 - "Hab Reference Resolution"
Cohesion: 0.07
Nodes (78): IndexedState, active_faction_councilors(), active_modules_in_sectors(), calculate_hab_slots(), calculate_world_wars(), candidate_module_monthly_delta(), control_point_summary(), councilor_attribute_breakdown() (+70 more)

### Community 13 - "Hab UI and Resources"
Cohesion: 0.21
Nodes (8): NationPriorityValidityTests, _boolean(), evaluate_priority_validity(), _number(), Any, Shared value-only nation priority validity contract., Evaluate only validity; callers precompute mechanics-specific derived inputs., _unknown()

### Community 14 - "CLI Save Loading"
Cohesion: 0.23
Nodes (30): build_index(), load_save(), print_json(), match_named(), command_ai_fleet_diagnostics(), command_councilor(), command_export(), command_faction() (+22 more)

### Community 15 - "AI Fleet Diagnostics"
Cohesion: 0.17
Nodes (30): add_state(), AIFleetDiagnosticsTests, ref(), synthetic_index(), _age_days(), calculate_ai_fleet_diagnostics(), _entry_id(), _explicit_blockers() (+22 more)

### Community 16 - "Topbar and Research UI"
Cohesion: 0.16
Nodes (29): apply_effect_modifiers(), effect_modifier_delta(), faction_effect_contexts(), find_faction_state(), load_hab_module_catalog(), Load packaged normalized hab modules; absence or corruption is fatal., raw_state_id(), ref_id() (+21 more)

### Community 17 - "Org Planner Tests"
Cohesion: 0.18
Nodes (5): eligibility_fixture(), org(), OrgPlanTests, profile(), ref()

### Community 18 - "Hab Module Planning"
Cohesion: 0.11
Nodes (17): pytest_collection_modifyitems(), MechanicsRegistryTests, CoverageResolver, CoverageResolvers, mechanic_diagnostics(), mechanic_rule_test(), MechanicRule, Audited Terra Invicta mechanics registry shared by code, tests and diagnostics. (+9 more)

### Community 19 - "Research UI Tests"
Cohesion: 0.21
Nodes (6): add_human_player(), add_state(), build_mission_control_fixture(), build_research_fixture(), ref(), ResearchUiTests

### Community 20 - "Module Construction Analysis"
Cohesion: 0.19
Nodes (25): Rules, AdvisorProfile, _apply_segment(), _canonical_priority(), _condition(), _condition_met(), _control_point_snapshot(), ControlPointPolicy (+17 more)

### Community 21 - "Hab Plan Tests"
Cohesion: 0.18
Nodes (3): add_state(), HabPlanTests, ref()

### Community 22 - "Parser Reliability Design"
Cohesion: 0.14
Nodes (18): Location Data Fail-Closed Contract, Packaged Catalog Strategy, TI Parser Reliability and Provenance, Player Identity and Packaged Module Catalog, Strict Human-Player Resolution, Metric-Specific Prior-Module Semantics, Unified Hab Module Effective State, CP Capacity and Event-Based Resource Forecast (+10 more)

### Community 23 - "Package Runtime Migration"
Cohesion: 0.17
Nodes (16): Package-only reliability master plan, Runtime and verification boundary, Exact scenario strictness, Runtime dependency audit phase, Raw-loader regression guard, Manifest and catalog integrity, Strict catalog foundation phase, Structured calculation dependencies (+8 more)

### Community 24 - "Hab Power Tests"
Cohesion: 0.25
Nodes (3): add_state(), HabPowerTests, ref()

### Community 25 - "Org Eligibility Design"
Cohesion: 0.18
Nodes (15): Org eligibility output master plan, Shared owner eligibility contract, Org eligibility rule discovery, Faction-scoped nation interest, Councilor owner trait rules, Enriched candidate eligibility contract, Org eligibility diagnostics implementation, Broken Earth eligibility evidence (+7 more)

### Community 26 - "Nation Projection Audit"
Cohesion: 0.40
Nodes (5): Fail-Closed Calculation Dependencies, Package-Only Runtime, Runtime Dependency and Silent-Fallback Audit, Audited Package-Only Projection, Strict Runtime Research Rows

### Community 27 - "Catalog Verification Tests"
Cohesion: 0.13
Nodes (9): ProjectionCoverageTests, ArmyProjectionState, combine_coverage(), MetricDependencyTracker, MetricEvidence, _names(), Execution-derived metric coverage and provenance for nation projection., Replace evidence for calculated outputs with their actual input graph. (+1 more)

### Community 28 - "Nation Claims Tests"
Cohesion: 0.21
Nodes (20): calculate_nation_claims(), _catalog_claim_metadata(), _catalog_for_scenario(), _deep_merge(), _nation_summary(), _number_or_none(), _numeric_rule(), Any (+12 more)

### Community 29 - "Org Parity Tests"
Cohesion: 0.29
Nodes (5): org_state(), ParserOrgParityTests, profile(), ref(), save_state()

### Community 30 - "CLI Command Dispatch"
Cohesion: 0.20
Nodes (8): ModuleType, NationProjectionCliTests, build_parser(), main(), ArgumentParser, Command-line parser and dispatch for the Terra Invicta save parser., build_parser(), main()

### Community 31 - "Hab Module Documentation"
Cohesion: 0.14
Nodes (13): All Normally Buildable Human Modules, Energy Lab, Fission Pile, TIHabModuleTemplate Source, Hab Support Cost, High-Value Recommendation Inputs, Module Economic Metrics, Outpost Core (+5 more)

### Community 34 - "Project README Concepts"
Cohesion: 0.24
Nodes (10): Current and projected MC separation, Core runtime catalogs phase, Normalized effect trait and org catalogs, Nation claims diagnostics phase, Claim mechanics evidence boundary, Terra Invicta Save Parser README, Fail-closed calculations, Modular parser architecture (+2 more)

### Community 35 - "Mission Control Projection"
Cohesion: 0.27
Nodes (10): Queued control centers master plan, Signed mission control headroom, Mission control omission discovery, Hab planning mission control gap, Prior module remains active during upgrade, Current-queue mission control implementation, Mission control effect consistency, Target versus prior template queue delta (+2 more)

### Community 36 - "Package Runtime Audit"
Cohesion: 0.25
Nodes (7): Base-Plus-Overlay Named Template Merge, Scenario Rules and DLC Template Overlays, Canonical Research Prerequisite Logic, Faction Projects, Global Techs, Sparse Exact Scenario Overrides, Terra Invicta Research Catalog

### Community 37 - "Scenario Rule Design"
Cohesion: 0.28
Nodes (9): Campaign-Start GDP Scaling, Canonical Scenario Identity, Scenario-Specific Parser Rules, Broken Earth Rule Values, Scenario Identity and Rule Model, Installed Save Validation, Scenario Regression and Save-File Verification, Campaign-Start GDP CP-Maintenance Fix (+1 more)

### Community 38 - "Graphify Memory Model"
Cohesion: 0.36
Nodes (9): Dense Durable Agent Notes, Hierarchical Topic Folders, mem:core Root, Memory Maintenance, Memory Reference Graph, Memory Reference Syntax, Progressive Discovery, Stable Memory Update Threshold (+1 more)

### Community 41 - "Hab Slot Tests"
Cohesion: 0.46
Nodes (4): add_state(), build_hab_fixture(), HabSlotSummaryTests, ref()

### Community 42 - "Package Runtime Tests"
Cohesion: 0.50
Nodes (4): PackageOnlyRuntimeTests, Path, ref(), state()

### Community 43 - "Project Analysis Tests"
Cohesion: 0.32
Nodes (3): add_state(), ProjectAnalysisTests, ref()

### Community 44 - "Research Plan Tests"
Cohesion: 0.32
Nodes (3): add_state(), ref(), ResearchPlanTests

### Community 45 - "Recommendation Evidence Design"
Cohesion: 0.29
Nodes (7): Diagnostics separated from recommendations, Org recommendation filtering phase, Eligible councilors as recommendation basis, Candidate source normalization, Missing org templates fail closed, AI fleet diagnostics phase, Observed derived suspected and unknown layers

### Community 46 - "Project Tooling Configuration"
Cohesion: 0.29
Nodes (7): Gitignore-Aware File Filtering, Local Project Overrides, Python Language Server, Serena Project Configuration, TI_Parser Project, UTF-8 Text Encoding, Writable Project Configuration

### Community 47 - "Hab Parser Parity Tests"
Cohesion: 0.60
Nodes (4): add_state(), build_parity_fixture(), HabParserParityTests, ref()

### Community 48 - "Raw Loader Guard Tests"
Cohesion: 0.50
Nodes (3): Counter, runtime_raw_loader_calls(), RuntimeRawLoaderGuardTests

### Community 49 - "Broken Earth Greece Query"
Cohesion: 0.83
Nodes (4): Broken Earth Greece Naming Query, Dominion of Hellas, Owned-Region Union Naming Trigger, Visions of Greatness Claim Activation

### Community 51 - "CLI Namespace"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 52 - "Community 52"
Cohesion: 0.09
Nodes (8): context(), NationProjectionEconomyTests, NationProjectionPlanTests, NationProjectionSchedulerTests, NationProjectionTransactionTests, state(), unity_context(), unity_state()

### Community 53 - "Community 53"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 54 - "Community 54"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 55 - "Community 55"
Cohesion: 0.33
Nodes (15): body_max_radius_km(), body_mean_radius_km(), build_catalog(), file_sha256(), main(), normalize_body(), normalize_collection(), normalize_navigable() (+7 more)

### Community 56 - "Community 56"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 57 - "Community 57"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 58 - "Community 58"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 59 - "Community 59"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 60 - "Community 60"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 61 - "Community 61"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 62 - "Community 62"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 63 - "Community 63"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 64 - "Community 64"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 65 - "Community 65"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 66 - "Community 66"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 67 - "Community 67"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 68 - "Community 68"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 69 - "Community 69"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 70 - "Community 70"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 71 - "Community 71"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 72 - "Community 72"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 73 - "Community 73"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 74 - "Community 74"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 75 - "Community 75"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 76 - "Community 76"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 77 - "Community 77"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 78 - "Community 78"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 79 - "Community 79"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 80 - "Community 80"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 81 - "Community 81"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 82 - "Community 82"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 83 - "Community 83"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 84 - "Community 84"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 85 - "Community 85"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 86 - "Community 86"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 87 - "Community 87"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 88 - "Community 88"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 89 - "Community 89"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 90 - "Community 90"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 91 - "Community 91"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 92 - "Community 92"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 93 - "Community 93"
Cohesion: 0.27
Nodes (11): main(), parse_args(), Namespace, compact_number(), parse_languages(), Any, Path, read_localization_file() (+3 more)

### Community 94 - "Community 94"
Cohesion: 0.14
Nodes (14): Audited build, Completion-specific boundaries, Fail-closed completion rules, Fail-Closed Priority Coverage, Metric dependency evidence, Nation projection mechanics audit, Rule index, Shared live priority validity (+6 more)

### Community 95 - "Community 95"
Cohesion: 0.20
Nodes (9): Completion, Global Validation Expectations, Issue Target And Scope Summary, Known Risks And Assumptions, Package-only reliability and diagnostics, Phase Dependencies, Phase Order, Source Of Truth Decisions (+1 more)

### Community 96 - "Community 96"
Cohesion: 0.51
Nodes (4): CatalogGeneratorTests, Path, write_json(), write_text()

### Community 97 - "Community 97"
Cohesion: 0.22
Nodes (8): Global Validation Expectations, Include queued control centers in projected mission control, Issue Target And Scope Summary, Known Risks And Assumptions, Phase Dependencies, Phase Order, Source Of Truth Decisions, Strategy

### Community 98 - "Community 98"
Cohesion: 0.22
Nodes (8): Global Validation Expectations, Issue Target And Scope Summary, Known Risks And Assumptions, Nation priority and conditional Advisor projection, Phase Dependencies, Phase Order, Source Of Truth Decisions, Strategy

### Community 99 - "Community 99"
Cohesion: 0.22
Nodes (8): Global Validation Expectations, Issue Target And Scope Summary, Known Risks And Assumptions, Org Eligibility Output, Phase Dependencies, Phase Order, Source Of Truth Decisions, Strategy

### Community 100 - "Community 100"
Cohesion: 0.22
Nodes (8): Global Validation Expectations, Issue Target And Scope Summary, Known Risks And Assumptions, Phase Dependencies, Phase Order, Scenario-specific parser rules, Source Of Truth Decisions, Strategy

### Community 102 - "Community 102"
Cohesion: 0.22
Nodes (8): Global Validation Expectations, Issue Target And Scope Summary, Known Risks And Assumptions, Phase Dependencies, Phase Order, Source Of Truth Decisions, Strategy, TI Parser reliability and provenance

### Community 104 - "Community 104"
Cohesion: 0.13
Nodes (37): cache_key(), candidate_save_dirs(), candidate_templates_dirs(), _catalog_module_to_template(), clean_number(), faction_is_human_player(), file_fingerprint(), find_latest_save() (+29 more)

### Community 105 - "Community 105"
Cohesion: 0.33
Nodes (5): Allowed raw IO, Final audit result, Normal-runtime raw sources, Runtime dependency and silent-fallback audit, Silent correctness degradation

### Community 106 - "Community 106"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 107 - "Community 107"
Cohesion: 0.50
Nodes (3): Answer, Q: BE 시나리오에서 Visions of Greatness 이후 Greece 국명이 바뀌고 모든 클레임 지역을 획득하면 다시 바뀌는가?, Source Nodes

### Community 109 - "Community 109"
Cohesion: 0.22
Nodes (9): Shared Mechanics Rule Registry, Strict and Observational Validation Boundary, Nation Priority and Conditional Advisor Projection, Mechanics Audit Registry and Data Catalog, CLI Faction Contribution and Diagnostics Integration, Hypothetical Advisor Policy, Target-Nation Faction Contribution, Projection Regression Verification (+1 more)

### Community 110 - "Community 110"
Cohesion: 0.05
Nodes (95): AdvisorProfile, CalculationDependencyError, PriorityValidityResult, RuntimeError, Path, RuntimeCatalogTests, write_json(), canonical_json_bytes() (+87 more)

### Community 111 - "Community 111"
Cohesion: 0.07
Nodes (61): annotate_candidate_opportunity_costs(), build_snapshot(), calculate_hab_ui(), candidate_affordable(), candidate_focus_score(), completion_datetime(), councilor_is_income_active(), faction_public_opinion() (+53 more)

### Community 112 - "Community 112"
Cohesion: 0.21
Nodes (20): _active_human_ideologies(), calibrate_rest_state_context(), _cohesion_dynamic_impact(), _democracy_cohesion_transform(), _distance(), _event_schedule(), _expire_faction_effects(), _faction_ideology_target() (+12 more)

### Community 113 - "Community 113"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 117 - "Community 117"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 118 - "Community 118"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 119 - "Community 119"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 120 - "Community 120"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 121 - "Community 121"
Cohesion: 0.14
Nodes (13): Acceptance criteria, Affected files, Decision log, Goal, Implementation steps, Manual smoke tests, Non-goals, Outcomes / Retrospective (+5 more)

### Community 122 - "Community 122"
Cohesion: 0.22
Nodes (10): adviser_attribute_bonus_from_values(), mission_control_contribution_from_values(), nation_monthly_research_from_values(), proportional_cp_contribution(), Return the audited nation Advise stat/100 bonus with descending rank decay., _base_ip(), _contribution(), _metrics() (+2 more)

### Community 123 - "Community 123"
Cohesion: 0.52
Nodes (7): ControlPointProjectionState, _diversity_bonus(), _effective_pips(), _priority_valid(), _record_and_fix_control_point(), _refresh_advisor_evidence(), _run_investment_transaction()

### Community 124 - "Community 124"
Cohesion: 0.50
Nodes (3): Answer, Q: pytest -q에서 mechanics registry test_ids의 tests namespace import가 실패한다, Source Nodes

## Knowledge Gaps
- **662 isolated node(s):** `IndexedState`, `Counter`, `Namespace`, `Namespace`, `Namespace` (+657 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **11 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `IndexedState` connect `Hab Reference Resolution` to `Scenario Rules Tests`, `Save Indexing Core`, `Nation State Calculations`, `Research Planning Pipeline`, `Income Dependency Model`, `Community 104`, `Projection and Simulation Helpers`, `Organization Planner`, `Habitat Mechanics`, `Community 110`, `AI Fleet Diagnostics`, `CLI Save Loading`, `Topbar and Research UI`, `Community 114`, `Community 111`, `Nation Claims Tests`?**
  _High betweenness centrality (0.048) - this node is a cross-community bridge._
- **Why does `Rules` connect `Module Construction Analysis` to `Nation Projection Engine`, `Community 123`, `Hab Reference Resolution`, `Hab UI and Resources`, `Community 110`, `Community 111`, `Community 112`, `CLI Save Loading`, `Hab Module Planning`, `Community 52`, `Catalog Verification Tests`?**
  _High betweenness centrality (0.045) - this node is a cross-community bridge._
- **Why does `Any` connect `Community 111` to `Save Indexing Core`, `Nation State Calculations`, `Research Planning Pipeline`, `Projection and Simulation Helpers`, `Hab Reference Resolution`, `CLI Save Loading`, `Community 110`, `Topbar and Research UI`, `Module Construction Analysis`?**
  _High betweenness centrality (0.030) - this node is a cross-community bridge._
- **Are the 11 inferred relationships involving `Any` (e.g. with `CatalogError` and `RuntimeCatalogs`) actually correct?**
  _`Any` has 11 INFERRED edges - model-reasoned connections that need verification._
- **Are the 34 inferred relationships involving `IndexedState` (e.g. with `AdvisorProfile` and `CalculationDependencyError`) actually correct?**
  _`IndexedState` has 34 INFERRED edges - model-reasoned connections that need verification._
- **Are the 11 inferred relationships involving `IndexedState` (e.g. with `CatalogError` and `RuntimeCatalogs`) actually correct?**
  _`IndexedState` has 11 INFERRED edges - model-reasoned connections that need verification._
- **Are the 2 inferred relationships involving `ref_id()` (e.g. with `faction_effect_expirations_for_projection()` and `_nation_ui_control_point_weights()`) actually correct?**
  _`ref_id()` has 2 INFERRED edges - model-reasoned connections that need verification._